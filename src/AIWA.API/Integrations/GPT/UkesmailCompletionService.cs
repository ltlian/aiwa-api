...
public class UkesmailCompletionService(IOptions<OpenAIOptions> options, ILogger<UkesmailCompletionService> logger, IOpenAiHttpClient openAiHttpClient) : IUkesmailCompletion
{
    ...
    public IAsyncEnumerable<CreateChatCompletionStreamResponse> GetUkesMailStreamingAsync(IEnumerable<string> promptExtras, CancellationToken cancellationToken = default)
    {
        ...
        var model = OpenAIModelMap.ToModel(options.Value.Model ?? GPT35_turbo);
        var chatCompletionsOptions = GetChatCompletionsOptions(promptExtras, model, 1, stream: true);
    ...
    {
        var model = OpenAIModelMap.ToModel(options.Value.Model ?? GPT35_turbo);
        var chatCompletionsOptions = GetChatCompletionsOptions(promptExtras, model, 1);
        if (logger.IsEnabled(LogLevel.Information))
    ...
    private static CreateChatCompletionRequest GetChatCompletionsOptions(IEnumerable<string> promptExtras, OpenAIModel model, int choiceCount = 1, bool stream = false)
    {
