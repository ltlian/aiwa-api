namespace AIWA.API.Audio
{
    ...
    public class Mp3Frame
    {
        ...
        public static async Task<Mp3Frame?> LoadFromStreamAsync(Stream input) => await LoadFromStreamAsync(input, true);
        ...
        /// </remarks>
        /// <returns>A valid MP3 frame, or null if none found</returns>
        public static async Task<Mp3Frame?> LoadFromStreamAsync(Stream stream, bool readData)
        {
            //byte[] headerBytes = new byte[4];
            var headerBytes = new byte[4].AsMemory();
            int bytesRead = await stream.ReadAsync(headerBytes);
            ...
            while (!IsValidHeader(headerBytes.Span, frame))
            {
                // shift down by one and try again
                headerBytes.Span[0] = headerBytes.Span[1];
                headerBytes.Span[1] = headerBytes.Span[2];
                headerBytes.Span[2] = headerBytes.Span[3];
                bytesRead = await stream.ReadAsync(headerBytes);
                if (bytesRead < 1)
                {
                    return null;
                }
            ...
            if (readData)
            {
                ...
                bytesRead = await stream.ReadAsync(frame.RawData.AsMemory(4, bytesRequired));
        ...
        /// </summary>
        private static bool IsValidHeader(Span<byte> headerBytes, Mp3Frame frame)
        {
